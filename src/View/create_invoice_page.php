...
<!--  -->
		...
		<?php foreach ($contacts->getContacts() as $contact): ?>
            <option value=<?php echo $contact['person_id']?>><?php echo $contact['person_id'].$contact['person_first_name']." ".$contact['person_last_name'] ?></option>
        <?php endforeach ?>
        <!--   -->
        </select>
    </div>
